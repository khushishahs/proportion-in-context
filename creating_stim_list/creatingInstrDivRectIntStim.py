...
colorOptions = ["#eb9626", "dodgerblue"]
...
def create_horizontal_line(w, unitH, size, x0, y0):
    ...
    for i in range(size + 1):
        y = y0 + i * lineDist
        # AS: draws a line from coordinate (x0,y) to (x0+w, y)
...
for v in range(1, 3): #to make two sets
    ...
    for m in range(0, len(colorOptions)): # to make a set for each color to be dominant, to be counterbalanced
        ...
        for j in range(0, len(stimList)):
            ...
            create_horizontal_line(width, unitHeight, set1, x0, y0_r1)
            create_horizontal_line(width, unitHeight, set2, x0, y0_r2)
            ...
            fileName = "DivRectInt_" + str(hueNames[m]) + "_" + str(set1) + "_" + str(set2) +  ".png"
